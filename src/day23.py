...
def traverse(grid: typing.Dict[typing.Tuple[int, int], str], len_x_top: int, prior_pos: typing.Dict[str, int], total: int = 0, _min: int = 0) -> int:
    grid = grid.copy()
    ...
    for (x, y), elem in grid.items():
        if elem in AMPHIPODS_COST and grid.get((x, y-1), '.') == '.':
            ...
            elem_dest = [(_x,0) for _x in range(1, len_x_top - 1) if _x not in AMPHIPODS_TO_Y_IDX.values()] if y > 0 else [elem_dest_max_pnt]
            ...
            for _pnt, _total in next_steps:
                ...
                if _pnt == elem_dest_max_pnt:
                    ...
                    _grid[_pnt] = elem
                n_min = traverse(_grid, len_x_top, prior_pos, _total, _min)
                _min = min(_min, n_min) if _min else n_min
    return _min


def run(lines: typing.List[str]) -> int:
    grid = {(x, y): e for y, l in enumerate(lines) for x, e in enumerate(l) if e in list(AMPHIPODS_COST) + ['.']}    
    return traverse(grid, len(lines[0]), dict())
