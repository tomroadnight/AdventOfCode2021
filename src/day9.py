...
def part_a_b() -> typing.Tuple[int, int]:
    ...
    low = reduce(lambda acc, i: acc + [(i, lines[i[0]][i[1]] + 1)] if all((lines[i[0]][i[1]] < z[1] for z in adj_vals(i))) else acc, [(y, x) for x in range(len(lines[0])) for y in range(len(lines))], [])
    low_sum = sum(map(lambda x: x[1], low))

    basin_lengths = list()
    for low_point, low_val in low:
        next_points = list((i for i in adj_vals(low_point) if i[1] >= low_val and i[1] < 9))
        seen = [low_point]
        while next_points:
            ...
            if point in seen: 
                continue
            seen.append(point)
            next_points.extend(list((i for i in adj_vals(point) if i[1] >= point_val and i[1] < 9)))
